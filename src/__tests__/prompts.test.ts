...
describe('QA_SYSTEM_PROMPT', () => {
  it('질문 유형별 상담형 답변 구조와 읽기 쉬운 분량을 강제', () => {
    expect(QA_SYSTEM_PROMPT).toContain('6~8개의 완성된 문장');
    expect(QA_SYSTEM_PROMPT).toContain('상황이 복잡할 때만 9문장까지 허용');
    expect(QA_SYSTEM_PROMPT).toContain('5문장 이하의 단답형 답변이나 10문장 이상의 장황한 답변은 피하세요');
    expect(QA_SYSTEM_PROMPT).toContain('마지막 격려 문장은 필요할 때만 한 문장');
    ...
    expect(QA_SYSTEM_PROMPT).toContain('전체 답변은 3~5개 문단');
  });
});
