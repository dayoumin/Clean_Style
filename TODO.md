...
현재 목표는 이름·자유서술·서버 저장·점수·대표 유형 판정이 없는 학생 인식·교육 홍보용 시연물을 먼저 완성하는 것이다.
모든 학생을 공통 `관점 탐색가`로 표현하고, 4개 캐릭터는 등급이 아닌 행동×이유 성찰을 돕는 관점 동료로만 사용한다.
...
- [ ] 행동과 이유를 따로 합산하지 않고 장면별 행동×이유 조합으로 관점 지도·다른 관점 질문을 결정
- [ ] 결과 레이더는 숫자·등급 없는 `관점 지도`로 구현하고 모든 4개 관점 동료를 같은 비중으로 표시
- [ ] 결과에 `다른 관점 보기 → 한 장면 다시 생각하기 → 다음 행동 고르기` 교육 흐름 포함
- [ ] 공유는 개인 결과 URL이 아닌 활동 초대 링크만 제공하고 URL에 문항·선택·축 내부값을 넣지 않음
